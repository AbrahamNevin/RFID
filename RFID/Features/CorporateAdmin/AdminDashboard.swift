    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: AtelierTheme.spacingL) {
                ...
                // KPI Grid
                KPIWidgetGrid(analytics: container.analyticsService)
                
                // AI Insights
                AIInsightPanel(aiService: container.aiService)
...
struct KPIWidgetGrid: View {
    ...
    var body: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: AtelierTheme.spacingM) {
            KPIWidget(title: "Net Revenue", value: "$\(String(format: "%.1f", analytics.globalNetRevenue / 1000000))M", trend: "+12.4%", icon: "dollarsign.circle")
            KPIWidget(title: "Inv. Accuracy", value: "\(String(format: "%.2f", analytics.inventoryAccuracy * 100))%", trend: "+0.05%", icon: "checkmark.shield")
            KPIWidget(title: "Global Shrink", value: "\(String(format: "%.2f", analytics.globalShrinkRate * 100))%", trend: "-2.1%", icon: "arrow.down.right.circle")
    ...
    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: AtelierTheme.spacingM) {
                ...
                if let insight = aiService.latestInsights.first {
                    Text(insight)
...
struct RegionalSalesChart: View {
    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: AtelierTheme.spacingM) {
